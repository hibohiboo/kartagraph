import { desktopApps } from '@/utils/desktop'
...
const deskReducer = createReducer(defState, (builder) => {
  builder.addCase(createAction('DESKREM'), (state, action) => {
    state.apps = state.apps.filter((x) => x.name != action.payload)
    localStorage.setItem(
      'desktop',
      JSON.stringify(state.apps.map((x) => x.name)),
    )
  })
})
